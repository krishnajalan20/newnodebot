...
const { BaseScene, Stge } = Scenes
const { enter, leave } = Stage
const stage = new Stage()
const rateLimit = require('telegraf-ratelimit');
var bot_token = '5170164710:AAH-LbSK4yHlBTFotbD-rbjhxVzNtWczsIE'; //YOUR BOT TOKEN HERE
...
stage.register(mkey)
const mid = new BaseSene('mid')
stage.register(mid)
const comment = new BaseScene('comment')
stage.register(comment)
var regex = new RegExp('.*')
const admin_id = 5002860771;
const mongo_url = 'mongodb+srv://DATA1:jP0GRPXHEORTE62W@cluster0.enchj.mongodb.net/myFirstDatabase?retryWrites=true&w=majority'; // Your Mongo URL Here
const buttonsLimit = {
    window: 1000,
    limit: 1,
    onLimitExceeed: (ctx, next) => {
...
mongo.connect(mongo_url, { useUnifiedTopology: true }, (err, client) => {
    ...
    db = clent.db('Giveaway_Lifafa_llBot');
    bot.telegram.deleteWebook().then(success => {
...
bot.start(async (ctx) => {
    try {
        ...
        let admin = await db.collection('admindb').find({ admin: "admin" }).toArray()
        if (admin.length == 0) {
            db.collection('admindb').inser
            tOne({ admin: "admin", ref: 1, cur: 'INR', paychannel: '@jsjdkkdkdhsjdk', bonus: 0.1, minimum: 1, botstat: 'Active', withstat: 'ON', subwallet: 'NOT SET', MKEY: 'NOT SET', MID: 'NOT SET', channels: [] })
            ctx.replyWithMarkdown(
                '*😅Restart Bot With /start*'
            )
        }
...
bot.on("contact", async(ctx)=> {
  try {
    ...
    if (ctx.update.message.forward_from){
      bot.telegram.sendMessage(ctx.from.id,"*⚠️Seems Like This Is Not Your Contact*",{parsmode:"markdown"})
      return
    }
    if(cont.startsWith("91") || cont.startsWith("+91")){
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ctx.replyWithMarkdown(
                    '*🏡 Welcome To Main Menu*', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],[' Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
                ...
                if (('inviter' in userdata[0]) && !('referred' in config[0])) {
                    ...
                    db.collection('balance').updatee({ userID: userdata[0].inviter }, { $set: { balance: final } }, { upsert: true })
                }
            } else {
                mustjoin(ctx)
            }
...
bot.hears('📘 Task Complete & 25₹ Earn', async(ctx) => {
    try {
        let text = '<b>🔥Claim Giveaway\n\n🔥 Link - https://cutt.ly/7GICyNk\n\n🔥 Claim Fast\n\n➡️ join more loot & Giveaway  - @Technical_Moneys</b>';
        await bot.telegram.sendPhoto(ctx.from.id,"https://siasky.net/VAC7zP9VUsuH_jpD-ZHcC-jQESwi934w-c7hp625m_Xhmw",{caption : text , parse_mode:'html'}).catch(err => console.log(err))
    } catch (error) {
        console.log(error)
    }
})
...
bot.hears('💰 Balance', async (ctx) => {
    try {
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ctx.replyWithMarkdown(
                    '*🙌🏻 User = ' + ctx.from.first_name + '\n\n💰 Balance = ' + ub.toFixed(3) + ' ' + currency + '\n\n🪢 Invite To Earn More*', { reply_markup: { keyboard: [['📘 Task Comple 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
            } else {
                mustjoin(ctx)
            }
...
bot.hears('👫 Invite', async (ctx) => {
    try {
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ctx.replyWithMarkdown(
                    '*🙌🏻 User =* [' + ctx.from.first_name + '](tg://user?id=' + ctx.frod + ')\n\n*🙌🏻 Your Invite Link = https://t.me/' + ctx.botInfo.username + '?start=' + ctx.from.id + ' \n\n🪢 Invite To ' + refer + ' ' + currency + ' Per Invite*', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
            } else {
                mustjoin(ctx)
            }
            ...
            if (flag == channel.length) {
                ...
                if (('inviter' in userdata[0]) && !('referred' in config[0])) {
                  if('verified' in config[0]){
                    ...
                    let ref = refer * 1
                    let final = ref + cur
                    bot.telegram.sensage(userdata[0].inviter, "*💰" + refer + " " + currency + " Added To Your Balance*", { parse_mode: 'markdown' })
                    bot.telegram.sendMessage(ctx.from.id, "*💹 To Check Who Invited You, Click On '✅ Check'*", { parse_mode: 'markdown', reply_markup: { inline_keyboard: [[{ text: "✅ Check", callback_data: "check" }]] } })
                    db.collection('allUsers').updateOne({ userID: ctx.from.id }, { $set: { inviter: userdata[0].inviter, referred: 'DONE' } }, { upsert: true })
        ...
        if (bots == 'Active') {
            if (withs == 'ON') {
                ...
                if (flag == channel.length) {
                    ...
                    if (ub < mini_with) {
                        ctx.replyWithMarkdown(
                            '*⚠️ Must Own AtLeast ' + mini_with + ' ' + currency + ' To Make Withdrawal*', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                        )
                    } else if (!data[0].wallet) {
                        ctx.replyWithMarkdown(
                            '*⚠️ Set Your Wallet Using : *`🗂 Wallet`', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                        )
                    } else {
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ...
                if (statdata.length == 0) {
                    ...
                    ctx.reply(
                        '<b>📊 Bot Live Stats 📊\n\n📤 Total Payouts : 0 ' + currency + '\n\n💡 Total Users: ' + members.length + ' Users\n\n🤟 Bot Maker: @I_Am_Bot_makers</b>' , { parse_mode: 'html', reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                    )
                } else {
...
broadcast.hears(regex, async (ctx) => {
    try {
        if (ctx.message.text == '⛔ Cancel') {
            ...
            ctx.replyWithMarkdown(
                '*📣 Broadcast Sent To: ' + users.length + ' Users*', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
            )
    ...
    try {
        ...
        if (flag == cha.length) {
            if (ctx.message.text == '⛔ Cancel') {
                ...
                ctx.replyWithMarkdown(
                    '*🗂 Wallet Address Set To: *\n`' + ctx.message.text + '`', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
            }
        } else {
            mustjoin(ctx)
            ...
            if (flag == channel.length) {
                ...
                if (ctx.message.text == '⛔ Cancel'){
                  ctx.replyWithMarkdown(
                        ...
                        '*⛔ Withdrawal Cancelled*', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                    ...
                    ctx.replyWithMarkdown(
                        '*⛔ Only Numeric Value Allowed*', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                    )
                    await ctx.scene.leave('onWithdraw')
                    return 0;
                } else if (ctx.message.forward_from){
                  ctx.replyWithMarkdown(
                        '*⛔ Forwards Are Prohibited*', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                    ...
                    ctx.replyWithMarkdown(
                        '*⛔ Entered Amount Is Greater Than Your Balance*', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                    )
                    await ctx.scene.leave('onWithdraw')
                    return 0;
                    ...
                    ctx.replyWithMarkdown(
                        ...
                        '*⚠️ Minimum Withdrawal Is ' + mini_with + ' ' + currency + '*', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }

                    )
                    await ctx.scene.leave('onWithdraw')
                    return 0;
                } else if (ctx.message.text > 1000){
                  ctx.replyWithMarkdown(

                        '*⚠️ Maximum Withdrawal Is 1000 ' + currency + '*', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
  ...
  try{
    ...
    if(toWith == 0){
      ...
      return 0;
        ...
        ctx.deleteMessage()
        ctx.replyWithMarkdown( 
                        "*✅ New Withdrawal Processed ✅\n\n🚀Amount : " + toWith + " " + currency + "\n⛔ Wallet :* `" + wallet + "`\n*💡 Bot: @" + ctx.botInfo.username + "*", {parse_mode:'markdown', reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } } 
                    )
...
bot.action("cancel",async(ctx)=> {
  try{
     ...
     ctx.deleteMessage()
     ctx.replyWithMarkdown( 
                        "*❌ Withdrawal Cancelled *", {parse_mode:'markdown', reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } } 
...
tgid.hears(/^[0-9]+$/, async (ctx) => {
    try {
        if (ctx.message.text == '⛔ Cancel') {
            ...
            if (!data[0]) {
                ctx.replyWithMarkdown(
                    '*⛔ User Is Not Registered In Our Database *', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
            } else {
                ...
                ctx.reply(
                    '<b>🫂 User : <a href="tg://user?id=' + ctx.message.text + '">' + ctx.message.text + '</a>\n⛔ User Id</b> : <code>' + ctx.message.text + '</code>\n\n<b>💰 Balance : ' + bal + '\n🗂 Wallet : </b><code>' + add + '</code>\n<b>👫 Inviter : </b><code>' + invite + '</code>', { parse_mode: 'html', reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
            }
        }
...
incr.hears(regex, async (ctx) => {
    try {
        if (ctx.message.text == '⛔ Cancel') {
            ...
            ctx.reply(
                '<b>💰 Balance Of <a href="tg://user?id=' + user + '">' + user + '</a> Was Increased By ' + amount + '\n\n💰 Final Balance = ' + final + '</b>', { parse_mode: 'html', reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
            )
...
chnl.hears(regex, async (ctx) => {
    try {
        ...
        if (ctx.message.text == '⛔ Cancel') {
            ...
            ctx.reply(
                '<b>🗂 Channel Added To Bot : ' + ctx.message.text + '</b>', { parse_mode: 'html', reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
            )
        } else {
            ctx.replyWithMarkdown(
                '*⛔ Channel User Name Must Start With "@"*', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
            )
        }
    ...
    try {
        ...
        if (ctx.message.text == '⛔ Cancel') {
            ...
            if (contains("" + ctx.message.text + "", chan)) {
                ...
                ctx.reply(
                    '<b>🗂 Channel Removed From Bot : ' + ctx.message.text + '</b>', { parse_mode: 'html', reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
            } else {
                ctx.reply(
                    '<b>⛔ Channel Not In Our Database</b>', { parse_mode: 'html', reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
            }
        } else {
            ctx.replyWithMarkdown(
...
paychnl.hears(regex, async (ctx) => {
    try {
        ...
        if (ctx.message.text == '⛔ Cancel') {
            ...
            ctx.reply(
                '<b>🗂 Pay Channel Set To : ' + ctx.message.text + '</b>', { parse_mode: 'html', reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
            )
        } else {
            ctx.replyWithMarkdown(
                '*⛔ Channel User Name Must Start With "@"*', { reply_markup: { keyboard: [['📘 Task Complete & 25₹ Earn'],['🎁 Bonus','💰 Balance','👫 Invite'], [ '🗂 Wallet','💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
...
function sleep(in_sec) {
    return new Promise(solve => setTimeout(resolve, in_sec * 1000));
};
function paytm(wallet, amount, subwallet, mkey, mid, comment) {
    ...
    var paytmParams = {};
    paytmParams["subwalletGuid"] = subwalle
    paytmParams["orderId"] = order;
    paytmParams["beneciaryPhoneNo"] = wallet;
    ...
    PaytmChecksum.generateSignature(post_data, mkey).then(function (checksum) {
        ...
        var options = {
            ...
            path: '/bpay/pi/v1/disburse/order/wallet/gratification',
            port: 443,
            method: 'POST',
            headers: {
